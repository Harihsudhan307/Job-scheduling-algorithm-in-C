...
printf("AFTER SUBTRACTING FROM ROWS:\n");
for (i=0;i<size1;++i)
 { for (j=0;j<size2;++j)
    {
     printf("%d ",Array[i][j]);}
...
printf("AFTER SUBTRACTING FROM COLUMNS:\n");
for (i=0;i<size1;++i)
 { for (j=0;j<size2;++j)
    {
     printf("%d ",Array[i][j]);}
...
for (y=0;y<size1;++y)
  for (x=0;x<size2;++x)
    if (Result[y][x])
      printf("row%d and column%d are connected in the assignment\n",y+1,x+1);
}
